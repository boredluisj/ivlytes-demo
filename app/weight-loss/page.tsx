    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <AnimateOnScroll>
            <span className="inline-block text-xs font-semibold uppercase tracking-widest text-[#2DD4BF] mb-4">Medical Weight Loss</span>
            ...
            <p className="text-xl text-[#9BA8BB] max-w-xl mb-4">
              Semaglutide &amp; Tirzepatide — starting at $50/shot. No consultation fee. Physician supervision included.
            </p>
            <p className="text-base text-[#9BA8BB]/70 max-w-lg mb-8">
            ...
            <a href="https://www.vagaro.com/ivlytes/book-now" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-7 py-3.5 bg-[#2DD4BF] text-[#0B1120] font-semibold rounded-xl hover:bg-[#2DD4BF]/90 hover:shadow-[0_0_30px_rgba(45,212,191,0.4)] transition-all">
              ...
              <AnimateOnScroll key={med.name} delay={i * 0.1}>
                <GlassCard className="p-8 h-full flex flex-col" hover>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-sora font-bold text-[#F0EDE8]">{med.name}</h3>
                  ...
                  <p className="text-sm text-[#9BA8BB] mb-6 leading-relaxed">{med.desc}</p>
                  <ul className="space-y-2.5 flex-grow">
                    {med.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-sm text-[#9BA8BB]">
            ...
            {[
              ...
              <AnimateOnScroll key={s.stat} delay={i * 0.1}>
                <div className="text-center p-6 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <TrendingDown className="w-6 h-6 text-[#2DD4BF] mx-auto mb-3" />
                  <div className="text-3xl font-sora font-bold mb-1" style={{ background: "linear-gradient(135deg, #2DD4BF, #2DD4BF)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{s.stat}</div>
                  <div className="text-sm font-medium text-[#F0EDE8] mb-1">{s.label}</div>
                  <div className="text-xs text-[#9BA8BB]">{s.sub}</div>
                </div>
              </AnimateOnScroll>
            ))}
          </div>
      ...
      <section className="py-16 bg-[#0B1120]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimateOnScroll>
            <div className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-[#2DD4BF]/10 border border-[#2DD4BF]/30 text-[#2DD4BF] mb-4">Partner Offer</div>
            <h2 className="text-2xl font-sora font-bold text-[#F0EDE8] mb-4">Save 20% on ProLon</h2>
            <p className="text-[#9BA8BB] mb-6">The ProLon 5-Day Fasting Mimicking Diet amplifies GLP-1 results by accelerating fat loss and cellular renewal. Our patients get 20% off through our partner link.</p>
      ...
      <div className="py-4 bg-[#0B1120] border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-xs text-[#9BA8BB]/60 text-center">IV-LYTES does not accept insurance. All services are cash-pay. These services have not been evaluated by the FDA and are not intended to diagnose, treat, cure, or prevent any disease. Individual results vary. Please consult your physician.</p>
        </div>
      </div>
