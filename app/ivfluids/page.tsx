    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <AnimateOnScroll>
            <span className="inline-block text-xs font-semibold uppercase tracking-widest text-[#2DD4BF] mb-4">
              IV Hydration & Nutrition
            </span>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-sora font-bold text-[#F0EDE8] mb-6 leading-tight">
              IV Hydration &<br />
            ...
            <p className="text-xl text-[#9BA8BB] max-w-xl">
            ...
            <div className="flex flex-wrap gap-3 mt-8">
              {["9 Drip Formulas", "From $99", "Same-Day Available", "Physician Supervised"].map((b) => (
                <span key={b} className="px-3 py-1 text-xs rounded-full bg-[#2DD4BF]/10 border border-[#2DD4BF]/20 text-[#2DD4BF]">
                  {b}
                </span>
              ))}
            </div>
      ...
      <section className="py-16 bg-[#0B1120]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimateOnScroll>
            <p className="text-lg text-[#9BA8BB] leading-relaxed text-center">
              Unlike oral supplements, IV therapy bypasses your digestive system entirely. When you swallow a vitamin C
              pill, your body absorbs 20–30% of it. With IV delivery, that number becomes <strong className="text-[#F0EDE8]">100%</strong>.
      ...
      {/* Drip Menu */}
      <section className="py-24 bg-[#0B1120]" aria-labelledby="drips-heading">
              ...
              <AnimateOnScroll key={drip.name} delay={i * 0.05}>
                <GlassCard hover className="p-6 h-full flex flex-col">
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-sora font-semibold text-[#F0EDE8] text-base">{drip.name}</h3>
                    <span className="text-sm font-semibold text-[#2DD4BF] flex-shrink-0 ml-2">{drip.price}</span>
                  </div>
                  <p className="text-sm text-[#9BA8BB] leading-relaxed flex-grow">{drip.description}</p>
                  <div className="flex items-center gap-1.5 mt-4 pt-4 border-t border-white/[0.06]">
                    <Clock className="w-3 h-3 text-[#9BA8BB]" />
                    <span className="text-xs text-[#9BA8BB]">30 min</span>
                  </div>
                </GlassCard>
              </AnimateOnScroll>
            ))}
      ...
      {/* Boosters */}
      <section className="py-16 bg-[#0B1120]" aria-labelledby="boosters-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {boosters.map((b, i) => (
              <AnimateOnScroll key={b.name} delay={i * 0.04}>
                <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.04] border border-white/[0.07] hover:border-[#2DD4BF]/20 transition-colors">
                  <span className="text-sm text-[#F0EDE8]">{b.name}</span>
                  <span className="text-sm font-semibold text-[#2DD4BF]">{b.price}</span>
                </div>
              </AnimateOnScroll>
            ))}
          </div>
            ...
            {steps.map((s, i) => (
              <AnimateOnScroll key={s.step} delay={i * 0.1}>
                <div className="text-center">
                  <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 text-xl font-sora font-bold text-[#0B1120]"
