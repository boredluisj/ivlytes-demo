...
function TestimonialCard({ name, initials, rating, text, service }: typeof testimonials[0]) {
  return (
    <div className="flex-shrink-0 w-[340px] p-6 rounded-xl bg-white/[0.04] border border-white/[0.07] backdrop-blur-xl mx-3">
      ...
      {/* Text */}
      <p className="text-sm text-[#A0AEC0] leading-relaxed mb-5 italic">&ldquo;{text}&rdquo;</p>
      {/* Author */}
      <div className="flex items-center gap-3">
        <AvatarPlaceholder initials={initials} size="sm" />
        <div>
          <div className="text-sm font-semibold text-[#F5F5F5]">{name}</div>
          <div className="text-xs text-[#00D9FF]">{service}</div>
        </div>
      </div>
    </div>
  );
...
export default function Testimonials() {
  ...
  return (
    <section className="py-24 bg-[#080C10] overflow-hidden" aria-labelledby="testimonials-heading">
